 ...
 */
class MailSendSubscriber extends \Shopware\Core\Content\MailTemplate\Subscriber\MailSendSubscriber implements EventSubscriberInterface
{
    ...
    public function __construct(AbstractMailService $emailService, EntityRepositoryInterface $mailTemplateRepository, MediaService $mediaService, EntityRepositoryInterface $mediaRepository, EntityRepositoryInterface $documentRepository, DocumentService $documentService, LoggerInterface $logger, EventDispatcherInterface $eventDispatcher, EntityRepositoryInterface $mailTemplateTypeRepository, Translator $translator, LanguageLocaleCodeProvider $languageLocaleProvider, $froshEventDispatcher)
    {
    ...
    {
        return [self::ACTION_NAME => 'sendMail'];
    }
    /**
    ...
    public function sendMail(Event $event) : void
    {
        ...
        $this->froshEventDispatcher->dispatch(new \Frosh\TemplateMail\Event\MailDataBagFilter($data, $mailTemplate, $event->getContext()));
        $data->set('mediaIds', []);
        ...
        if ($data->has('templateId')) {
            try {
                $this->mailTemplateTypeRepository->update([['id' => $mailTemplate->getMailTemplateTypeId(), 'templateData' => $this->getTemplateData($mailEvent)]], $mailEvent->getContext());
            } catch (\Throwable $e) {
                // Dont throw errors if this fails // Fix with NEXT-15475
            }
        }
        try {
            $this->emailService->send($data->all(), $event->getContext(), $this->getTemplateData($mailEvent));
    ...
    private function buildAttachments(BusinessEvent $event, MailTemplateEntity $mailTemplate, MailSendSubscriberConfig $config) : array
    {
        ...
        if (!empty($config->getDocumentIds())) {
            ...
            foreach ($entities as $document) {
                ...
                $attachments[] = ['content' => $document->getFileBlob(), 'fileName' => $document->getFilename(), 'mimeType' => $document->getContentType()];
            }
        }
        return $attachments;
    }
    private function injectTranslator(MailActionInterface $event) : bool
    {
        ...
        $this->translator->injectSettings($event->getSalesChannelId(), $event->getContext()->getLanguageId(), $this->languageLocaleProvider->getLocaleForLanguageId($event->getContext()->getLanguageId()), $event->getContext());
        return true;
    }
}